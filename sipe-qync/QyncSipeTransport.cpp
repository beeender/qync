...
QyncSipeTransport::QyncSipeTransport(struct sipe_core_public *sipe_public,
    ...
    :QObject(parent), mSocket(0), mReadBuf(0, 0)
{
    memset(&mSipeTransportCon, 0, sizeof(mSipeTransportCon));
    mSipeTransportCon.thiz = this;
    mSipeTransportCon.pub.type = setup->type;
    mSipeTransportCon.pub.user_data = setup->user_data;

    mConSetup = new sipe_connect_setup;
    memcpy(mConSetup, setup, sizeof(sipe_connect_setup));
    mSipeCorePublic = sipe_public;
